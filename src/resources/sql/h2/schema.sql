# 用户基本表
CREATE TABLE IF NOT EXISTS PEI_USER(
  ...
  ADDRESS_ID    BIGINT                        COMMENT '收货信息ID,取值表PEI_USER_ADDRESS'
) comment='用户基本信息表';

# 用户收货信息表
CREATE TABLE if not exists PEI_USER_ADDRESS(
  ID BIGINT AUTO_INCREMENT PRIMARY KEY NOT NULL ,
  ADDRESS_TYPE_NAME VARCHAR(255)              COMMENT '收获信息名称',
...
# 食谱信息表
create table if not exists pei_cookbook(
  ID bigint auto_increment primary key not null ,
...
# 食材及调味品信息表
create table if not exists pei_cook_material(
  ID bigint auto_increment primary key not null ,
  ...
  material_number   int(10)                   comment '库存数量',
...
# 食谱与食材关联表
create table if not exists pri_cook_material_rela(
  ID bigint auto_increment primary key not null ,
...
# 品牌信息表
create table if not exists pei_brand(
  ID bigint auto_increment primary key not null ,
...
# 订单表
create table if not exists pei_order(
  ID bigint auto_increment primary key not null ,
